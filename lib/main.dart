  ...
  Future<void> _getUserAction() async {
    ...
    if (Platform.isAndroid) {
      try {
        ...
        log(result.toString());
        lastNotificationData = Map<String, dynamic>.from(result['last_notification_data']);
    ...
    Future.delayed(Duration.zero, () {
      ...
      FirebaseMessaging.onMessageOpenedApp.listen((message) {
        ...
        if (_userAction == 'Accept') {
          Navigator.push(context, MaterialPageRoute(builder: (_) => const AcceptCallScreen()));
        } else {
          Navigator.push(context, MaterialPageRoute(builder: (_) => const RejectCallScreen()));
        }
      });
    });
    super.initState();
